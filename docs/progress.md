...
## Sep 13 — Roadmap decisions locked; hands (2a) started on a branch

Worked through the Decide items in `future.md` with David. Locked: (1) the multi-stage promotion rule is accepted as written (promote when a step's output feeds a later step, a stage needs its own fixture, or a step needs isolation or a narrower tool set) and the proof is to birth one real multi-stage villager after 2a lands; (4) stay on the managed Vercel Connect app, one app, humans pre-create channels, with a new small build item: the midwife gives the human explicit pre-birth instructions; (5) "asked" means an explicit human `@villager` mention and nothing else, everything else in the channel is context only, recorded as [ADR 0004](adrs/0004-channel-scoped-trigger-mention-only.md) which supersedes ADR 0002 and preserves the anti-loop rule; (6) proactive villagers deferred until after hands; (7) multi-level memory and provenance deferred, community brain only. New open item 8: whether memory should be snapshotted to git at all (the git half of ADR 0003) is YAGNI until a reader of the public record exists, so `snapshot_memory` gets no trigger and the discussion happens later. Dropped: the Ren ping and the ShellOverflow post. David is flipping `message.channels` on the Connect trigger. Item 2a (hands at birth) started the same day in a separate session on `feature/hands-at-birth`. No code change on `main`.
